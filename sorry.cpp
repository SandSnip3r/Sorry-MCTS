  ...
  for (size_t i=0; i<playerCount; ++i) {
    for (size_t j=i+1; j<playerCount; ++j) {
      if (playerColors[i] == playerColors[j]) {
        throw std::runtime_error("Player " + std::string(sorry::toString(playerColors[i])) + " appears multiple times in the player list");
      }
    }
  }
...
bool Sorry::equalForPlayer(const Sorry &other, PlayerColor playerColor) const {
  ...
  for (size_t playerIndex=0; playerIndex<players_.size(); ++playerIndex) {
    const sorry::Sorry::Player &thisPlayer = players_.at(playerIndex);
    const sorry::Sorry::Player &otherPlayer = other.players_.at(playerIndex);
...
Sorry::Player& Sorry::getPlayer(PlayerColor playerColor) {
  ...
  throw std::runtime_error("Trying to get player data for an invalid player "+std::string(sorry::toString(playerColor)));
}
